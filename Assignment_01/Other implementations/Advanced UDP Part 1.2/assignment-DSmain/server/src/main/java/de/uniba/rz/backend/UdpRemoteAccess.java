...
public class UdpRemoteAccess implements RemoteAccess {
	...
	// public static List<List<Integer>> mainTicketStore = new ArrayList<>();
		...
		try (DatagramSocket serverSocket = new DatagramSocket(null)) {
			...
			while (true) {
				...
				try {
					...
					System.out.println("The initial conetent i ::::: " + content);
					if (content.equals("getAllTicket")) {
						...
						if (finalp != null) {
							...
							System.out.println(
									String.format("\t *** [SERVER]: This Package has been sent via %s Fragments",
											numberof_fragments));
		...
		resServerTicketList.forEach(ticket -> {
			...
			ticketDataList.forEach(item -> {
				...
				try (DatagramSocket sock = new DatagramSocket(null)) {
					...
					SocketAddress serverAddress = new InetSocketAddress(this.hostname, 1199);
