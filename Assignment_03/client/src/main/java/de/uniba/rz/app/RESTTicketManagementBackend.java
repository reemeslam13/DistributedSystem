...
public class RESTTicketManagementBackend implements TicketManagementBackend {
	...
	public void sendTicket(Ticket ticket) {
		...
		try {
			...
			Response response = (Response) client.target("http://localhost:9999").path("/tickets")// **in config file or
																									// arg
			...
			if (statusCode >= 200 && statusCode <= 299) {
				System.out.println("* Create New Ticket: " + ticket.getTopic());
			}
			System.out.println("Http Status: " + String.valueOf(statusCode) + ", Location: " + response.getLocation());
			System.out.println("-------------------");
		} // if
		finally {
	...
	public List<Ticket> filterTickets(String name, Type type) {
		...
		Response response = (Response) client.target("http://localhost:9999").path("/search") // http://localhost:9999/cats/5
				.queryParam("name", name).queryParam("type", type).request(MediaType.APPLICATION_JSON)
	...
	public List<Ticket> filterTickets(String name, Type type, int offset, int limit) {
		...
		Response response = (Response) client.target("http://localhost:9999").path("/search") // http://localhost:9999/cats/5
				.queryParam("name", name).queryParam("type", type).queryParam("offset", offset)
	...
	private void updateTicket(Ticket modifiedTicket) {
		...
		Response response = (Response) client.target("http://localhost:9999").path("/tickets/" + ticketid)
				.request(MediaType.APPLICATION_JSON).header("Content-Type", "application/json").put(entity);

		System.out.println(
				"* Update Ticket: " + modifiedTicket.getDescription() + "  To: " + modifiedTicket.getStatus().toString());

		System.out.println(" Http Status: " + String.valueOf(response.getStatus()) );
		System.out.println("-------------------");
	}
