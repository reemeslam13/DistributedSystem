...
public class SwingMainModel extends Observable {
    ...
    public List<Ticket> searchTicket(String name, Type type, int offset, int limit) throws TicketException {
		try {
			return backend.searchTickets(name, type, offset, limit);
    ...
    public List<Ticket> searchTicket(String name, Type type) throws TicketException {
		try {
			  ...
			  if (type == null) { return backend.getTicketsByName(name); } else { return
			  backend.getTicketsByNameAndType(name, type); }
