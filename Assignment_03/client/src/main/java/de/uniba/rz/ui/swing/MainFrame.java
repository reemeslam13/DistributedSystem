...
public class MainFrame extends JFrame implements Observer {
	...
	private MsgPassingMethod msgPassingMethod;

	public MainFrame(SwingMainController controller, SwingMainModel mainModel, MsgPassingMethod msgPassingMethod) {
		this.controller = controller;
		this.mainModel = mainModel;
		this.msgPassingMethod = msgPassingMethod;
	...
	private JPanel createSearchPanel() {
		...
		searchButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				catch(NumberFormatException ex) {offset = 0;}
				try { 
					limit = Integer.parseInt(limitField.getText());
				}
				catch(NumberFormatException ex) {limit = 0;}
				controller.searchTicket(searchField.getText(), (de.uniba.rz.entities.Type) typeBox.getSelectedItem(),offset,limit);
			}
		});
		...
		// Added by Azar
		if (msgPassingMethod != MsgPassingMethod.REST) {
